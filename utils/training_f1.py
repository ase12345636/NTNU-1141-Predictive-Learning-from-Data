...
def train_epoch_f1(model, train_loader, criterion, optimizer, device, scaler: GradScaler | None = None):
    """Train for one epoch with F1 score evaluation."""
    model.train()
    train_loss = 0
    train_preds = []
    train_labels = []
    ...
    return train_loss, train_f1_macro, train_f1_weighted

def validate_epoch_f1(model, val_loader, criterion, device):
    """Validate for one epoch with F1 score evaluation."""
    model.eval()
    val_loss = 0
    val_preds = []
    val_labels = []
    ...
    return val_loss, val_f1_macro, val_f1_weighted

def test_model_f1(model, test_loader, criterion, device):
    """Test model with F1 score evaluation and per-image inference time."""
    model.eval()
    test_loss = 0
    test_preds = []
    test_labels = []
    ...
    return test_loss, test_f1_macro, test_f1_weighted, test_preds, test_labels, test_f1_per_class, avg_ms
