...
def main():
    ...
    model = load_lsnet_model(num_classes=NUM_CLASSES, device=DEVICE, model_complexity=LSNET_COMPLEXITY)
    ...
    optimizer_unfreeze = torch.optim.AdamW(
        [
            {"params": model.head.parameters(), "lr": 1e-3},
            {
                ...
                "lr": 1e-4     # backbone 用較小 lr
            }
        ],
        weight_decay=1e-4
    )

    scheduler_freeze = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_freeze, mode='max', factor=0.5, patience=3
    )
    scheduler_unfreeze = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_unfreeze, mode='max', factor=0.5, patience=3
    )
    ...
    scheduler = scheduler_freeze

    for epoch in range(EPOCHS):
        if epoch == FREEZE_EPOCHS + 1:
            print("Unfreezing backbone...")
            unfreeze_lsnet_all(model)
            optimizer = optimizer_unfreeze
            scheduler = scheduler_unfreeze
