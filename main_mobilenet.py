...
# 輸出目錄（根據配置命名）
RESULTS_DIR = f'results_mobilenet_v{MOBILENET_VERSION}'
CHECKPOINTS_DIR = f'checkpoints_mobilenet_v{MOBILENET_VERSION}'
CHECKPOINTS_FILENAME = f'mobilenet_v{MOBILENET_VERSION}_best'
CHECKPOINTS_FILENAME_FINAL = f'mobilenet_v{MOBILENET_VERSION}_final'
...
def main():
    ...
    #------------------------------------
    # 2. Create MobileNet model
    #------------------------------------
    print("Loading MobileNet model...")
    model = load_mobilenet_model(num_classes=NUM_CLASSES, device=DEVICE, model_version=MOBILENET_VERSION)
