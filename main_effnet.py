...
TRAIN_RATIO = 0.8
CHECKPOINTS_FILENAME = 'effnet_final'
RESULTS_DIRNAME = 'results_effnet'
...
model = load_efficientnet_model(num_classes=NUM_CLASSES, device=DEVICE)
...
# Save results
save_model_weights(model, f'checkpoints/{CHECKPOINTS_FILENAME}.pth')
plot_learning_curves(history, f'{RESULTS_DIRNAME}/learning_curve.png')
save_training_history(history, f'{RESULTS_DIRNAME}/training_history.json')
...
model_size_mb = os.path.getsize(f'checkpoints/{CHECKPOINTS_FILENAME}.pth') / (1024 ** 2)
...
save_test_results(
    ...
    test_preds,
    test_labels,
    f'{RESULTS_DIRNAME}/test_results.json',
